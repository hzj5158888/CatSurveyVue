          ...
          <div class="ct-table-title">
            <el-row :span="24" type="flex" justify="space-between" align="middle">
              ...
              <el-col :span="20" style="text-align: right;">
                <el-button v-has-ct-role="'CTSURVEY_SUPER_ADMIN'" type="primary" size="medium" @click="openCreateUser()">添加用户</el-button>
              </el-col>
            </el-row>
          </div>
    ...
    <div>
      <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" append-to-body width="40%" >
        <el-form ref="userForm" :model="userForm" :rules="userFormRules" status-icon label-position="top">
          ...
          <el-form-item :label-width="formLabelWidth" label="账号状态" prop="status" class="ct-dialog-form-item">
          ...
          <el-form-item :label-width="formLabelWidth" label="设置密码" prop="pwd" class="ct-dialog-form-item" >
...
import {ctAdminUserList, ctUserCreate, ctUserDelete, ctUserUpdate} from '../../api/admin/admin-user'
...
export default {
  ...
  methods: {
    ...
    queryList (pageNo) {
      ...
      ctAdminUserList(this.pageSize, pageNo, status, loginName).then((response) => {
    ...
    handleDelete (index, row) {
      ...
      this.$msgbox.confirm('确认删除此用户吗？', '删除警告', {type: 'warning', confirmButtonText: '确认删除'}).then(() => {
        ...
        ctUserDelete(data).then((response) => {
      ...
      this.$refs['userForm'].validate((valid) => {
        if (valid) {
          ...
          if (id === null) {
            ...
            ctUserCreate(data).then((response) => {
              const httpResult = response.data
              if (httpResult.resultCode === 200) {
            ...
            ctUserUpdate(data).then((response) => {
              const httpResult = response.data
              if (httpResult.resultCode === 200) {
